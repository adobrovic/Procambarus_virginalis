...
%   TC_tL_path15000 = tempcorr(temp.tL_path15000, T_ref, pars_T);
%   TC_tWw_path15000 = tempcorr(temp.tWw_path15000, T_ref, pars_T);
...
% length-fecundity & weight-fecundity
  ...
  %% for experimental data
 % t-L, t-Ww and L-Ww data for control group in pathogen infection experiment
 F = f_exp;
 L_b = L_m * get_lb([g k v_Hb], F); Lw_b = L_b / del_M;
 Lw_i = L_m * (F - l_T)/ del_M;
 ...
 % L-Ww control
 ELWw_control = (LW_control(:,1) * del_M).^3 * (1 + F * ome);  % g, wet weight for the control group;
  
  
% % experimental t-L, t-Ww and L-Ww data for juvenile marbled crayfish exposed to pathogen conc. 7500 spores/ml
 pM_new = p_M * (1 + s_path7500);
 k_M_new = pM_new/ E_G;            % 1/d, somatic maintenance rate coefficient
 k_new = k_J/ k_M_new;            % -, maintenance ratio
 v_Hb_new = V_Hb * g^2 * k_M_new^3/ v^2;
 L_m_new = v/ k_M_new/ g;           % cm, maximum length

 F = f_exp;
 L_b = L_m_new * get_lb([g k_new v_Hb_new], F); Lw_b = L_b / del_M;
 Lw_i = L_m_new * (F - l_T)/ del_M;
 ir_B = 3/ k_M_new + 3 * F * L_m_new/ v; rT_B = TC_tL_path7500/ ir_B;  % d, 1/von Bert growth rate // % 1/d, von Bert growth rate
 % tL_path7500
 ...
 % LW_path7500
 ELWw_path7500 = (LW_path7500(:,1) * del_M).^3 * (1 + F * ome);  % g, wet weight for infected group;
 ...
 % pack to output
  ...
  prdData.LW_control = ELWw_control;
  ...
  prdData.LW_path7500 = ELWw_path7500;