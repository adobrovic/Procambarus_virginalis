...
data.ah = 20;    units.ah = 'd';    label.ah = 'age at hatching'; 	   bibkey.ah = 'VogtToll2004';  
  ...
  comment.ah = 'Embryonic period lasts 17-22days, depending on conditions. VogtTollScho2004; The embryonic period between spawning and hatching lasts 2–3weeks';
...
data.tp = 245 - 25;    units.tp = 'd';    label.tp = 'time since birth at puberty at 20°C';   bibkey.tp = 'VogtTollScho2004';
...
data.Lh  = 0.35;   units.Lh  = 'cm';  label.Lh  = 'total length at hatching'; bibkey.Lh  = 'VogtTollScho2004';
data.Lb  = 0.57;   units.Lb  = 'cm';  label.Lb  = 'total length at birth'; bibkey.Lb  = 'VogtTollScho2004'; 
data.Lp  = 4;   units.Lp  = 'cm';  label.Lp  = 'total length at puberty'; bibkey.Lp  = 'VogtTollScho2004'; 
data.Lp  = 4;   units.Lp  = 'cm';  label.Lp  = 'total length at puberty'; bibkey.Lp  = 'VogtTollScho2004'; 
...
data.CLi  = 4.5;   units.CLi  = 'cm';  label.CLi  = 'ultimate carapace length'; bibkey.CLi  = 'VogtFalc2015';
  comment.CLi = 'Carapace length; largest in lab and nature 4.9cm CL'; 
...
data.Ri = 450/(8*7); units.Ri = 'd';    label.Ri = 'max reprod rate';        bibkey.Ri = {'VogtFalc2015', 'VogtDorn2019','HossKoub2019'};
    comment.Ri = {'Temperature guessed; number of eggs based on larger crayfish in datasets (clutch size increases with the female size), reproducing every 8-9 weeks all year round under very good conditions; '...
        '150-270 eggs for larger crayfish in VogtTollScho2004, the largest clutches of marbled crayfish in the laboratory and the wild consisted of >720 eggs(VogtFalc2015 and refs) '};  
    temp.Ri = C2K(20);  units.temp.Ri = 'K'; label.temp.Ri = 'temperature';

data.E0 = 55.56 * 1.5/2.22; units.E0  = 'J'; label.E0  = 'initial energy content of the egg'; bibkey.E0  = {'RodrGarc2006', 'VogtTollScho2004'};
comment.E0 = 'Based on Redclaw crayfish: egg diameter (2.22 mm) and energy (55.56 J/egg) [RodrGarc2006], and egg diameter of marble crayfish of 1.5mm [VogtTollScho2004] ';
...
% According to VogtTollScho2004, yolk reserves are available at least for the first half juv3 stage, so we take 15 days from hatching to birth
 % t-L data
...
units.tCL30 = {'d', 'cm'};  label.tCL30 = {'time since birth', 'carapace length'}; bibkey.tCL30 = 'SeitVilp2005';
...
units.tCL25 = {'d', 'cm'};  label.tCL25 = {'time since birth', 'carapace length'}; bibkey.tCL25 = 'SeitVilp2005';
...
units.tCL20 = {'d', 'cm'};  label.tCL20 = {'time since birth', 'carapace length'}; bibkey.tCL20 = 'SeitVilp2005';
...
units.tWw30 = {'d', 'g'}; label.tWw30 = {'time since birth', 'wet weight'}; bibkey.tWw30 = 'SeitVilp2005';
...
units.tWw25 = {'d', 'g'}; label.tWw25 = {'time since birth', 'wet weight'}; bibkey.tWw25 = 'SeitVilp2005';
...
units.tWw20 = {'d', 'g'}; label.tWw20 = {'time since birth', 'wet weight'}; bibkey.tWw20 = 'SeitVilp2005';
...
weights.ab = 5 * weights.ab;  % more weight to age at birth
weights.ah = 5 * weights.ah;  % more weight to ah 
weights.Lh = 5 * weights.Lh;  % more weight to Lh 
weights.E0 = 10 * weights.E0;  % more weight to E_0
weights.tL1 = 10 * weights.tL1;  % more weight to tL for little ones
weights.tL2 = 10 * weights.tL2;  % more weight to tL for larger ones
weights.tCL30 = 0.01 * weights.tCL30;  % less weight to tricky temps, more weight to optima
weights.tWw30 = 0.01 * weights.tWw30;  % less weight to tricky temps, more weight to optima
weights.tCL15 = 0.01 * weights.tCL15;  % less weight to tricky temps, more weight to optima
weights.tWw15 = 0.01 * weights.tWw15;  % less weight to tricky temps, more weight to optima
weights.tCL20 = 10 * weights.tCL20;  % less weight to tricky temps, more weight to optima
weights.tWw20 = 10 * weights.tWw20;  % less weight to tricky temps, more weight to optima
weights.tCL25 = 10 * weights.tCL25;  % less weight to tricky temps, more weight to optima
weights.tWw25 = 10 * weights.tWw25;  % less weight to tricky temps, more weight to optima
...
%% Group plots
set1 = {'tL1','tL2'};
comment1 = {'Age - Length from VogtHube2008'};
set2 = {'LW_Vogt2010','LW_ParvTogo2017','LW_Ziza2015','LW_HossKoub2019'};
comment2 = {'Length-Wet weight data from 4 sources'};
set3 = {'LN_Vogt', 'LN_HossKoub2019'}; 
comment3 = {'length-# of eggs from 2 sources'};
set4 = {'tCL30', 'tCL25', 'tCL20', 'tCL15'};
comment4 = {'age - length at 15°C, 20°C, 25°C and 30°C'};
set5 = {'tWw30','tWw25','tWw20','tWw15'};
comment5 = {'age - wet weight at 15°C, 20°C, 25°C and 30°C'};
    
metaData.grp.sets = {set1,set2,set3,set4,set5};
metaData.grp.comment = {comment1,comment2,comment3,comment4,comment5};

%% Discussion points
D1 = 'Model type: *std* instead of abj, based on the fact that freshwater crayfish, unlike marine decapods, have no larval stage, and the overwhelmingly good fit to growth data';
...
metaData.bibkey.F1 = 'VogtTollScho2004'; 
F2 = 'Reproduction exclusively by parthenogenesis, no male specimen found (therefore classification problematic)';
metaData.bibkey.F2 = 'VogtTollScho2004'; 
...
metaData.bibkey.F3 = 'VogtTollScho2004'; 
...
metaData.bibkey.F4 = 'VogtTollScho2004'; 
...
'author = {Hossain, Md Shakhawate and Kouba, Antonin and Buriè, Miloš}, ' ...
'year = {2019}, ' ...
'title = {Morphometry, size at maturity, and fecundity of marbled crayfish (Procambarus virginalis)}, ' ...
...
'author = {Parvulescu, Lucian and Togor, Andrei and Lele, Sandra and Scheu, Sebastian and ?inca, Daniel and Panteleit, Jörn}, ' ...
'year = {2017}, ' ...
'title = {First established population of marbled crayfish Procambarus fallax (Hagen, 1870) f. virginalis (Decapoda, Cambaridae) in Romania}, ' ...
...
'author = {Velisek, Josef and Stara, Alzbeta and Koutnik, Dalibor and Zusková, Eliška and Kouba, Antonín}, ' ...
'year = {2014}, ' ...
'title = {Effect of prometryne on early life stages of marbled crayfish (Procambarus fallax f. virginalis)}, '...
...
'author = {Vogt, Günter and Tolley, Laura}, ' ...
'year = {2004}, ' ...
...
metaData.biblist.(bibkey) = ['''@', type, '{', bibkey, ', ' bib, '}'';'];
%
bibkey = 'VogtTollScho2004'; type = 'Article'; bib = [ ...  
'author = {Vogt, Günter and Tolley, Laura and Scholtz, Gerhard}, ' ...
'year = {2004}, ' ...
...
'author = {Vogt, Günter and Huber, Martin and Thiemann, Markus and Boogaart, Gerald and Schmitz, Oliver and Schubart, Christoph}, ' ...
'year = {2008}, ' ...
...
'author = {Vogt, Günter and Falckenhayn, Cassandra and Schrimpf, Anne and Schmid, Katharina and Hanna, Katharina and Panteleit, Jörn and Helm, Mark and Schulz, Ralf and Lyko, Frank}, ' ...
'year = {2015}, ' ...
...
'author = {Vogt, Günter and Dorn, Nathan and Pfeiffer, Michael and Lukhaup, Chris and Williams, Bronwyn and Schulz, Ralf and Schrimpf, Anne}, ' ...
'year = {2019}, ' ...
'title = {The dimension of biological change caused by autotriploidy: A meta-analysis with triploid crayfish Procambaru virginalis and its diploid parent Procambarus fallax}, ' ...
'journal = {Zoologischer Anzeiger}, ' ...
'volume = {281}, ' ...
'pages = {53-67}, ' ...
...
'author = {Anita Žižak}, ' ...
'year = {2015}, ' ...
'title = {Organosomatski indeksi invazivne vrste \textit{Procambarus fallax} ({H}agen, 1870) f. virginalis iz jezera Šoderica (Koprivnica)}, ' ...
